...
import { TrendingUp, DollarSign, Percent, ArrowDown, ArrowUp } from 'lucide-react';
import type { AssetPortfolio } from '@/lib/types';
import { mockAssets, calculateAssetPortfolio } from '@/lib/mock-data';
          ...
          <div className="p-3 rounded-md bg-muted/30">
           ...
           <div className="p-3 rounded-md bg-muted/30">
            <div className="text-sm text-muted-foreground mb-1">Total P/L %</div>
             <div className={cn(
                "text-xl font-semibold flex items-center",
                portfolio.totalProfitLossPercentage >= 0 ? "text-green-500" : "text-red-500"
              )}>
               {portfolio.totalProfitLossPercentage >= 0 ? <ArrowUp className="w-4 h-4 mr-1" /> : <ArrowDown className="w-4 h-4 mr-1" />}
